...
public class Controller extends Application {
    ...
    private HashMap<String, String> VMPictures = new HashMap<String, String>();
    ...
    public void setActions(View view) {
        ...
        view.getBtnConfirmSpecCreateNewItem().setOnAction(e -> clickConfirmSpecCreateNewItem()); 
        view.getBtnSpecSelectItemPicture().setOnAction(e -> clickSpecSelectItemPic());
        view.getBtnBackSpecCreateNewItem().setOnAction(e -> clickBackSpecNewItem());
        ...
        view.getBtnBackPayItem().setOnAction(e -> clickBackPayItem());
        view.getBtnCollectSpecified().setOnAction(e -> clickCollectSpecified());
        view.getBtnCollectAll().setOnAction(e -> clickCollectAll());
        view.getBtnConfirmSpecVMPurchaseAnItem().setOnAction(e -> clickConfirmVMPurchaseAnItem());
    }
    ...
    public void clickBtnReplenishCashReg() {
        ...
        if(specialVM != null)
            view.getTextAmountStored().setText(specialVM.displayMoneyQty());
    ...
    public void clickConfirmRepriceItem() {
        ...
        if(specialVM != null) {
            try {
                ...
                if(specialVM.isSlotAssigned(slotNumber - 1)) {
                    if(!specialVM.repriceItem((double)price, slotNumber - 1)) {
                        Alert a = new Alert(Alert.AlertType.WARNING, 
                            "Reprice unsuccessful!", ButtonType.OK);
                        a.showAndWait();
                    } else {
                        Alert a = new Alert(Alert.AlertType.INFORMATION, 
                            "Successfully repriced " + specialVM.getSlotItem(slotNumber - 1).toString(), ButtonType.OK);
                        a.showAndWait();
                    }
                } else {
                    Alert a = new Alert(Alert.AlertType.WARNING, 
    ...
    public void clickPurchaseItem() {
        VMPictures.put("Sausage", "file:Sausage.png");
        VMPictures.put("Fried Egg", "file:Fried Egg.png");
        VMPictures.put("Asado Pork", "file:Asado Pork.png");
        VMPictures.put("Beef Brisket", "file:Beef Brisket.png");
        VMPictures.put("Chicken Leg", "file:Chicken Leg.png");
        VMPictures.put("Duck Leg", "file:Duck Leg.png");
        VMPictures.put("Vegetables", "file:Vegetables.png");
        VMPictures.put("Noodle Base", "file:Noodle Base.png");
        ...
        if(regularVM != null) {
            ...
            for(int i = 0; i <= noOfSlots - 1; i++) {
                if(!regularVM.isSlotAssigned(i))
                    ...
                    if(VMPictures.containsKey(regularVM.getSlotItem(i).getName())) {
                        ImageView buttonIcon = new ImageView(new Image(VMPictures.get(regularVM.getSlotItem(i).getName())));
                        buttonIcon.setFitWidth(70);
                        buttonIcon.setPreserveRatio(true);
                        view.getRegVMPurchaseButtons().get(i).setGraphic(buttonIcon);
                    } else if(!VMPictures.containsKey(regularVM.getSlotItem(i).getName()))
        ...
        if(specialVM != null) {
            view.getStage().setScene(view.makeMySpecVMPurchaseAnItem());
            int noOfSlots = Integer.parseInt(view.getTextSpecialVMNoOfSlots().getText());

            for(int i = 0; i <= noOfSlots - 1; i++) {
                if(!specialVM.isSlotAssigned(i)) {
                    view.getSpecVMPictures().get(i).setImage(new Image("file:NoPhoto.png"));
                    view.getSpecVMQuantities().get(i).setText("0");
                }
                else if(specialVM.isSlotAssigned(i)) {
                    if(VMPictures.containsKey(specialVM.getSlotItem(i).getName())) {
                        view.getSpecVMPictures().get(i).setImage(new Image(VMPictures.get(specialVM.getSlotItem(i).getName())));
                        view.getSpecVMInfo().get(i).setText(specialVM.getSlotItem(i).toString()
                            + "\nIs Sold Alone: " + specialVM.getSlotItem(i).getIsSoldAlone());
                    } else if(!VMPictures.containsKey(specialVM.getSlotItem(i).getName()))
                        view.getSpecVMPictures().get(i).setImage(new Image("file:NoPhoto.png"));
                }
            }
        }
    ...
    public void clickConfirmVMPurchaseAnItem() {
        try {
            int noOfSlots = Integer.parseInt(view.getTextSpecialVMNoOfSlots().getText());
            int[] slotQuantities = new int[noOfSlots];

            for(int i = 0; i <= noOfSlots - 1; i++) 
                slotQuantities[i] = Integer.parseInt(view.getSpecVMQuantities().get(i).getText());

            if(specialVM.isPurchaseValid(slotQuantities)) {
                view.getBtnConfirmPayItem().setOnAction(e -> clickConfirmSpecItem(slotQuantities));
                view.getStage().setScene(view.makePayItem());
                view.getTextPayItemInfo().setText(specialVM.computeOrderTotal(slotQuantities) + " Pesos");
            }
            else if(!specialVM.isPurchaseValid(slotQuantities)) {
                Alert c = new Alert(Alert.AlertType.WARNING, 
                    "Some items cannot be sold alone!", ButtonType.OK);
                c.showAndWait();
            }
        } catch(NumberFormatException e) {
            Alert c = new Alert(Alert.AlertType.WARNING, 
    ...
    public void clickConfirmSpecItem(int[] slotQuantities) {
        try {
            int[] money = new int[9];
            int totalAmount;
            boolean amountCheck = true;
            
            double price = specialVM.computeOrderTotal(slotQuantities);
            ...
            if(totalAmount < price || !amountCheck) {
                Alert c = new Alert(Alert.AlertType.WARNING, 
                ...
                Alert c = new Alert(Alert.AlertType.INFORMATION, 
                    specialVM.processPurchase(money, slotQuantities), ButtonType.OK);
                c.showAndWait();
            }
        } catch(NumberFormatException e) {
            Alert c = new Alert(Alert.AlertType.WARNING, 
    ...
    public void clickCreateNewItem() {
        ...
        if(specialVM != null)
            view.getStage().setScene(view.makeSpecSlotCreateNewItem());
    }
    ...
    public void clickConfirmSpecCreateNewItem() {
        if(specialVM != null) {
            try {
                String itemName = view.getTextNameSpecCreateItem().getText();
                double itemPrice = (double)Integer.parseInt(view.getTextPriceSpecCreateItem().getText());
                double itemCalories = (double)Integer.parseInt(view.getTextCaloriesSpecCreateItem().getText());
                boolean itemIsSoldAlone = Boolean.parseBoolean(view.getTextIsSoldAlone().getText());
                String itemPreparation = view.getTextPreparation().getText();

                if(!specialVM.isItemDuplicate(new Item(itemName, itemPrice, itemCalories, itemIsSoldAlone, itemPreparation))) {
                    specialVM.createNewItem(itemName, itemPrice, itemCalories, itemIsSoldAlone, itemPreparation);
                    Alert a = new Alert(Alert.AlertType.INFORMATION, 
                        "New item created: " + itemName + " (Price: " + itemPrice + ", Calories: " + itemCalories + 
                        ", Is Sold Alone: " + itemIsSoldAlone + ", Preparation: " + itemPreparation + ")",
                        ButtonType.OK);
                    a.showAndWait();
                }
            } catch(NumberFormatException e) {
    ...
    public void clickRegSelectItemPic() {
        try {
            ...
            if (file != null) {
                VMPictures.put(item.getName(), file.toString());
                Alert a = new Alert(Alert.AlertType.INFORMATION, 
    ...
    public void clickSpecSelectItemPic() {
        try {
            String itemName = view.getTextNameSpecCreateItem().getText();
            double itemPrice = (double)Integer.parseInt(view.getTextPriceSpecCreateItem().getText());
            double itemCalories = (double)Integer.parseInt(view.getTextCaloriesSpecCreateItem().getText());
            boolean itemIsSoldAlone = Boolean.parseBoolean(view.getTextIsSoldAlone().getText());
            String itemPreparation = view.getTextPreparation().getText();
            Item item = new Item(itemName, itemPrice, itemCalories, itemIsSoldAlone, itemPreparation);
            ...
            if (file != null) {
                VMPictures.put(item.getName(), file.toString());
                Alert a = new Alert(Alert.AlertType.INFORMATION, 
                    "Successfully set photo for " + item.toString() +
                    " isSoldAlone: " + itemIsSoldAlone + "- Preparation by: " + itemPreparation, ButtonType.OK);
                a.showAndWait();
            }
        } catch(NumberFormatException e) {
            Alert c = new Alert(Alert.AlertType.WARNING, 
    ...
    public void clickBackSpecNewItem() {
        view.getChoiceSlotNumber().getItems().clear();
        view.getChoiceItem().getItems().clear();
